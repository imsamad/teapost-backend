...
const uploadAssets = (0, utils_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!(req === null || req === void 0 ? void 0 : req.files) || filesKeys.length > 1)
        return next((0, utils_1.ErrorResponse)(400, 'Only single file allowed to upload'));
    ...
    if (!file.size > limitFileSize)
        return next((0, utils_1.ErrorResponse)(400, `File must be of less than ${limitFileSize} bytes`));
    ...
    const response = yield (0, cloudinary_1.uploadImageToCloudinary)(file === null || file === void 0 ? void 0 : file.tempFilePath, publicId, resource_type);
    let assetUploaded = {
    ...
    yield Asset_1.default.findByIdAndUpdate(user, {
        _id: user,
        $push: { [resource_type + 's']: assetUploaded },
    }, { upsert: true, new: true });
    return res.status(200).json({ result: [assetUploaded] });
}));
const uploadAssetsTwo = (0, utils_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const filesKeys = Object.keys(req.files);
    if (!(req === null || req === void 0 ? void 0 : req.files) || !filesKeys.length)
        return next((0, utils_1.ErrorResponse)(400, 'Provide data'));
    const limitFileSize = parseInt(process.env.LIMIT_FILE_SIZE, 10);
    const allFiles = filesKeys.map((key) => Array.isArray(req.files[key]) ? req.files[key] : [req.files[key]]);
    if (allFiles.length > 4)
        return next((0, utils_1.ErrorResponse)(400, 'Provide data'));
    if (!allFiles.every((file) => file.size > limitFileSize))
        return next((0, utils_1.ErrorResponse)(400, `Every data must be of less than ${limitFileSize} bytes`));
    let uploaders = allFiles.map((file) => {
        const resource_type = file.mimetype.includes('video')
            ? 'video'
            : file.mimetype.includes('image')
                ? 'image'
...
const uploadMultipleDiffTypesAssets = (0, utils_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c, _d;
